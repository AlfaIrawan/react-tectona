...
import {
  addIdeaFolderMember,
  removeIdeaFolderMember,
  updateIdeaFolderMemberRole,
  type IdeaFolderMemberRoleCode,
} from '@/lib/api/ideaFolderApi'
import { cn } from '@/lib/utils'
import { enterpriseCyanGradientActionButtonClass } from '@/lib/enterpriseButtonClasses'
import { fetchProjectWorkspaceDirectoryUsers } from '@/modules/projects/lib/projectWorkspaceMembers'
import { resolveFolderMemberDisplayName } from '@/modules/projects/lib/folderMemberIdentity'
import {
  ProjectMemberRoleSelect,
  projectMemberRoleLabel,
} from '@/modules/projects/components/ProjectMemberRoleSelect'
...
type ShareIdeaFolderDrawerProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  folder: IdeaBacklogFolder | null
}

export function ShareIdeaFolderDrawer({ open, onOpenChange, folder }: ShareIdeaFolderDrawerProps) {
  ...
  const [selectedRole, setSelectedRole] = useState<IdeaFolderMemberRoleCode>('member')
  const [submitting, setSubmitting] = useState(false)
  const [pendingUserId, setPendingUserId] = useState<string | null>(null)

  const members = useMemo(() => folder?.members ?? [], [folder])
  ...
  const refreshFolder = async () => {
    if (!folder) return
    await useIdeaFolderStore.getState().fetchFolders(folder.ownerId, folder.parentId)
  }
  ...
  const handleAddMember = async () => {
    ...
    setSubmitting(true)
    try {
      await addIdeaFolderMember(folder.id, { user_id: selectedUserId, role_code: selectedRole })
      await refreshFolder()
      setSelectedUserId(null)
      setSearchQuery('')
      addToast({
  ...
  const handleRemoveMember = async (userId: string, displayName: string) => {
    ...
    try {
      await removeIdeaFolderMember(folder.id, userId)
      await refreshFolder()
      addToast({
  ...
  const handleRoleChange = async (userId: string, roleCode: IdeaFolderMemberRoleCode) => {
    if (!folder) return
    setPendingUserId(userId)
    try {
      await updateIdeaFolderMemberRole(folder.id, userId, roleCode)
      await refreshFolder()
    } catch (error: unknown) {
      addToast({
  ...
  return createPortal(
    ...
    <div className="fixed inset-0 z-[1400]" onClick={(e) => e.stopPropagation()}>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="share-idea-folder-title"
      ...
      >
        <div className="flex items-start justify-between border-b border-border/70 px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 id="share-idea-folder-title" className="text-base font-semibold tracking-tight text-foreground">
                Share folder
              </h2>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-muted-foreground">
                          {member.roleName || projectMemberRoleLabel(member.roleCode)}
                        </p>
                      </div>
                      {isOwner ? (
                        ...
                        <div className="flex items-center gap-1.5">
                          <ProjectMemberRoleSelect
                            value={member.roleCode as IdeaFolderMemberRoleCode}
                            disabled={busy}
          ...
          <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-[0_8px_24px_-18px_rgba(15,23,42,0.12)] dark:border-slate-800/80 dark:bg-slate-950/50">
            ...
            <div className="mt-4 flex items-center gap-4">
              <Label htmlFor="idea-folder-member-role" className="shrink-0 text-muted-foreground">
                Role
              </Label>
              <ProjectMemberRoleSelect
                id="idea-folder-member-role"
                value={selectedRole}
                disabled={submitting}
