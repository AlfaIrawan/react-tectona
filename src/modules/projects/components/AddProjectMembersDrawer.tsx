...
import { enrichProjectWithIdentityNames } from '../lib/projectMemberIdentity'
...
type AddProjectMembersDrawerProps = {
  open: boolean
...
export function AddProjectMembersDrawer({
  open,
  onOpenChange,
  project,
  onProjectUpdated,
}: AddProjectMembersDrawerProps) {
  ...
  const [selectedRole, setSelectedRole] = useState<ProjectMemberRoleCode>('member')
  ...
  useEffect(() => {
    ...
    void fetchProjectWorkspaceDirectoryUsers()
      .then(({ users, displayNameByUserId: nameMap }) => {
        ...
        onProjectUpdated(enrichProjectWithIdentityNames(project, nameMap))
        if (users.length === 0) {
  ...
  const applyProjectApi = (apiProject: ProjectApi) => {
    const nextProject = enrichProjectWithIdentityNames(
      {
        ...project,
        ...
        updatedAt: apiProject.updated_date ?? apiProject.created_date,
      },
      displayNameByUserId,
    )
  ...
  const handleAddMember = async () => {
    ...
    setSubmitting(true)
    try {
      const apiProject = await addProjectMember(project.id, {
        user_id: selectedUserId,
        role_code: selectedRole,
      })
      applyProjectApi(apiProject)
      setSelectedUserId(null)
      setSearchQuery('')
      addToast({
        title: 'Member added',
        description: 'User has been added to this project.',
        variant: 'success',
      })
    } catch (error: unknown) {
      addToast({
  ...
  const handleRemoveMember = async (userId: string, displayName: string) => {
    ...
    try {
      const apiProject = await removeProjectMember(project.id, userId)
      applyProjectApi(apiProject)
      addToast({
        title: 'Member removed',
        description: `${displayName} was removed from the project.`,
        variant: 'success',
      })
    } catch (error: unknown) {
      addToast({
        title: 'Remove member failed',
        description: error instanceof Error ? error.message : 'Could not remove project member.',
        variant: 'error',
      })
    } finally {
  ...
  const handleRoleChange = async (userId: string, roleCode: ProjectMemberRoleCode) => {
    setPendingUserId(userId)
    try {
      const apiProject = await updateProjectMemberRole(project.id, userId, roleCode)
      applyProjectApi(apiProject)
    } catch (error: unknown) {
      addToast({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1400]">
      <button
        ...
        aria-label="Close add members drawer"
        disabled={submitting}
        onClick={() => {
      ...
      >
        <div className="flex items-start justify-between border-b border-border/70 px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 id="add-project-members-title" className="text-base font-semibold tracking-tight text-foreground">
                Share project
              </h2>
              <p className="mt-0.5 text-sm text-muted-foreground">
                Manage who can access <span className="font-medium text-foreground">{project.name}</span>.
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      ...
                      {isOwner ? (
                        ...
                        <div className="flex items-center gap-1.5">
                          <ProjectMemberRoleSelect
                            value={member.roleCode as ProjectMemberRoleCode}
                            disabled={busy}
          ...
          <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-[0_8px_24px_-18px_rgba(15,23,42,0.12)] dark:border-slate-800/80 dark:bg-slate-950/50">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Search for a member already registered in the Workspace, then set their project role.
            </p>
            ...
            <div className="mt-4 flex items-center gap-4">
              <Label htmlFor="project-member-role" className="shrink-0 text-muted-foreground">
                Role
              </Label>
              <ProjectMemberRoleSelect
                ...
                value={selectedRole}
                disabled={submitting}
                aria-label="Role for new project member"
